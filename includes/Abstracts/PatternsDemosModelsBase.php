...
class PatternsDemosModelsBase {
			...
			case 'patterns':
				...
				if ( is_array( $patterns ) && ! empty( $patterns ) ) {
					foreach ( $patterns as $key => $pattern ) {
						...
						$data->patterns[ $key ]->content = preg_replace( $regex, 'href="#"', $content );
					...
					foreach ( $demos as $demo_key => $demo ) {
						...
						if ( is_array( $pages ) && ! empty( $pages ) ) {
							foreach ( $pages as $key => $page ) {
								...
								$data->demos[ $demo_key ]->pages[ $key ]->content = preg_replace( $regex, 'href="#"', $content );
						...
						if ( is_object( $parts ) && ! empty( $parts ) ) {
							foreach ( $parts as $key => $part ) {
								...
								$data->demos[ $demo_key ]->parts->{$key} = preg_replace( $regex, 'href="#"', $part );
							}
						}
						...
						if ( is_object( $templates ) && ! empty( $templates ) ) {
							foreach ( $templates as $key => $template ) {
								...
								$data->demos[ $demo_key ]->templates->{$key} = preg_replace( $regex, 'href="#"', $template );
	...
	protected function fetch() {
		...
		$args = apply_filters( "omnipress_{$this->model_type}_model_remote_args", array() );
